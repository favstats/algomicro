...
+ The paper is in a somewhat weird stage where it's both 1.) too long (in the wrong places) and 2.) still needs some more details filled in (theory-wise). 
 
+ The parts I need most help with are the Intro, "theory" and conclusion. I find it a bit hard constructing a red thread throughout the paper and could use some help with finding the overall story I want to tell in this paper. 
 
+ Another thing I am struggling with is that I believe I may pre-registered myself into a little bit of a corner, with many individual hypotheses that maybe should have been research questions in the first place. Hindsight is 20/20. Is the way I’ve dealt with this issue in the paper confusing? Should I maybe deviate from the pre-registration and if so, how? 
 
+ The analysis part is also a bit long probably, so if someone has any suggestions how to better structure it, I am all ears. 
 
+ Finally, I am not super happy with the current title of the paper ("Paying to Win"), so maybe our creative juices together can come up with a better title 😊  
